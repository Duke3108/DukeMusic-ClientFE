        ...
        <div className='bg-[#121212] h-full rounded'>
                <div className='p-4 flex items-center justify-between'>
                    ...
                    <div className='flex items-center gap-3 rounded-full p-2 cursor-pointer hover:bg-[rgba(255,255,255,0.1)]'>
                        <img onClick={handleCreatePlaylist} className='w-5' src={assets.plus_icon} alt=''/>
                    </div>
                </div>
                ...
                ?   <div className='bg-[#242424] rounded text-[14px] mx-2 px-4 py-5 font-bold flex flex-col gap-1'>
                        ...
                        <div>
                            <button onClick={handleCreatePlaylist} className='px-4 py-1 bg-white text-black rounded-full mt-2 cursor-pointer'>Tạo danh sách phát</button>
                        </div>
                    </div>
                :   <div className="flex flex-col gap-2">
                    {playlistData.map((index,item) => (
                        <PlaylistItem key={index} name={item.name} image={item.imageUrl} id={item._id}/>
                    ))}
                    ...
                    <div className="p-4 block text-[14px] max-w-[336px] text-black bg-blue-400 w-[332px] rounded-lg font-bold absolute top-[15%] left-[20%]">
                        <div className="flex flex-col">
                            ...
                            <div className="mt-6 flex justify-between items-center">
                                <div></div>
                                <div className="gap-2">
                                    <button onClick={() => setOpen(false)} className="bg-transparent rounded-full px-4 py-2">Để sau</button>
